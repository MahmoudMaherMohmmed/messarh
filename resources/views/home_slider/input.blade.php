        ...
        <div class="tab-content">
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}" id="title{{ $language->short_code }}">
                    <input class="form-control" name="title[{{ $language->short_code }}]" value="@if ($home_slider) {!! $home_slider->getTranslation('title', $language->short_code) !!} @endif" />
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="col-sm-9 col-md-8 controls">
        <div class="fileupload fileupload-new" data-provides="fileupload">
            <div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
                @if($home_slider)
                    <img src="{{url($home_slider->image)}}" alt="" />
                @else
